...
class df_cuenca_subterranea(models.Model):
    _name = 'df.cuenca.subterranea'
    _inherit = 'df.cuenca.subterranea'
    _description = "HC Groundwater basin"
    ...
    def write(self, vals):
        ...
        super(df_cuenca_subterranea, self).write(vals)
        if vals.get('a0') or vals.get('a1') or vals.get('valor_precision') or vals.get(
                'recurso_explotacion'):
            pozo_ids_actualizar = self.ids
            ok = False
            if pozo_ids_actualizar:
                cuenca_obj.obtener_promedio_alturas(pozo_ids_actualizar, ok)
                cuenca_obj.obtener_promedio_alturas_formula(pozo_ids_actualizar, ok)
        return True


    @api.multi
    def search(self,args, offset=0, limit=None, order=None, count=False):
        usuario_loggeado = self.env['res.users'].search([('id', '=', self.env.uid)])
        state_ids = []
        for compannia in usuario_loggeado.company_ids:
            state_ids.append(compannia.state_id.id)
        args.append(['provincia_id.id', 'in', state_ids])
        return super(df_cuenca_subterranea, self).search(args, offset, limit, order, count)

    def _valores_rango_fechas(self, id, pozo_ids, anno):
        if not pozo_ids:
            pozo_obj = self.env['df.pozo']
            #pozo_ids = pozo_obj.search([],context={'exportar_filtro': 'cuenca', 'filtro': [[0, False, [id]]]}) #cambio
            pozo_ids = pozo_obj.with_context({'exportar_filtro': 'cuenca', 'filtro': [[0, False, [id]]]}).search([('cuenca_subterranea_id','=',id)]).ids #cambio
        ...
        query = """SELECT
                    ...
                    GROUP BY df_nivel_anual_pozo.anno"""

        self.env.cr.execute(query, ((tuple(pozo_ids)), anno))
        return self.env.cr.dictfetchall()

    def _valores_sin_rango_fechas(self, id, pozo_ids):
        if not pozo_ids:
            pozo_obj = self.env['df.pozo']
            pozo_ids = pozo_obj.with_context({'exportar_filtro': 'cuenca', 'filtro': [[0, False, [id]]]}).search([]) #cambio
        ...
        query = """SELECT
                    ...
                    WHERE
                          ...
                          df_pozo.id in %s
                    GROUP BY df_nivel_anual_pozo.anno"""
        self.env.cr.execute(query, ((tuple(pozo_ids)),))
        return self.env.cr.dictfetchall()

    def _calcular_media_cuenca_rango_fecha(self, ids, pozo_ids, fecha_inicio, fecha_fin):
        res = []
        ...
        for id in ids:
            ...
            while current_year <= anno_fin:
                    ...
                    if datos[0]['anno'] == anno_inicio:
                        if mes_inicio < 2:
                            if anno_inicio == anno_fin and mes_fin < 1:
                                ...
                                dict_media['id'] = id
                                arreglo_cuenca.append(dict_media)
                                current_year += 1
                                continue
                ...
                arreglo_cuenca.append(dict_media)
                current_year += 1
            res.append(arreglo_cuenca)
        return res

    def _calcular_media_cuenca_sin_rango_fecha(self, ids, pozo_ids):
        res = []
        for id in ids:
            arreglo_cuenca = []
    ...
    def calcular_media_aritmetica(self, ids,  pozo_ids, fecha_inicio, fecha_fin):
        if fecha_inicio and fecha_fin:
            if fecha_fin < fecha_inicio:
                ...
                fecha_inicio = temp
            res = self._calcular_media_cuenca_rango_fecha(ids, pozo_ids, fecha_inicio, fecha_fin)
        else:
            res = self._calcular_media_cuenca_sin_rango_fecha(ids, pozo_ids)
        return res
    ...
    def altura(self, ids,  pozo_ids, fecha_inicio, fecha_fin, tipo):
        ...
        if datos_vistas:
            for id in ids:
                if fecha_inicio == None and fecha_fin == None:
                    dict_max=self.max_min(ids, pozo_ids,None, None, tipo)
                else:
                    dict_max=self.max_min(ids, pozo_ids,fecha_inicio, fecha_fin,tipo)
        ...
        if datos_vistas:
            for id in ids:
                ...
                if fecha_inicio == None and fecha_fin == None:
                    ...
                    dict_max=self.max_min(ids,pozo_ids,fecha_inicio,fecha_fin,tipo)
                max=dict_max[0]['valor_max']
                rec_explotable = self.browse(id).recurso_explotacion
                if tipo == 'formula':
                    deltaH=2*(self.browse(id).promedio_h_periodo_formula)  #Para probar
                    coeficiente_calculado=(rec_explotable * 1000000)/(deltaH*area*1000000)
                else:
                    #deltaH=2*(self.browse(id).promedio_h_periodo) #Original
                    deltaH=2*(5)  #Para probar
                    coeficiente_calculado=(rec_explotable * 1000000)/(deltaH*area*1000000)
                ...
                for datos_vista in datos_vistas[temp]:
                    ...
                    if datos_vista.get('1'):
                        if max_fijo <= 0.0:
                            dict_altura['1']= ((coeficiente*(max-datos_vista['1'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['1']= ((coeficiente*(max_fijo-datos_vista['1'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('2'):
                        if max_fijo <= 0.0:
                            dict_altura['2']= ((coeficiente*(max-datos_vista['2'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['2']= ((coeficiente*(max_fijo-datos_vista['2'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('3'):
                        if max_fijo <= 0.0:
                            dict_altura['3']= ((coeficiente*(max-datos_vista['3'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['3']= ((coeficiente*(max_fijo-datos_vista['3'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('4'):
                        if max_fijo <= 0.0:
                            dict_altura['4']= ((coeficiente*(max-datos_vista['4'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['4']= ((coeficiente*(max_fijo-datos_vista['4'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('5'):
                        if max_fijo <= 0.0:
                            dict_altura['5']= ((coeficiente*(max-datos_vista['5'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['5']= ((coeficiente*(max_fijo-datos_vista['5'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('6'):
                        if max_fijo <= 0.0:
                            dict_altura['6']= ((coeficiente*(max-datos_vista['6'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['6']= ((coeficiente*(max_fijo-datos_vista['6'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('7'):
                        if max_fijo <= 0.0:
                            dict_altura['7']= ((coeficiente*(max-datos_vista['7'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['7']= ((coeficiente*(max_fijo-datos_vista['7'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('8'):
                        if max_fijo <= 0.0:
                            dict_altura['8']= ((coeficiente*(max-datos_vista['8'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['8']= ((coeficiente*(max_fijo-datos_vista['8'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('9'):
                         if max_fijo <= 0.0:
                            dict_altura['9']= ((coeficiente*(max-datos_vista['9'])*(area*1000000))/1000000)* cof_aprov
                         else:
                            dict_altura['9']= ((coeficiente*(max_fijo-datos_vista['9'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('10'):
                        if max_fijo <= 0.0:
                            dict_altura['10']= ((coeficiente*(max-datos_vista['10'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['10']= ((coeficiente*(max_fijo-datos_vista['10'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('11'):
                        if max_fijo <= 0.0:
                            dict_altura['11']=((coeficiente*(max-datos_vista['11'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['11']=((coeficiente*(max_fijo-datos_vista['11'])*(area*1000000))/1000000)* cof_aprov
                    if datos_vista.get('12'):
                        if max_fijo <= 0.0:
                            dict_altura['12']=((coeficiente*(max-datos_vista['12'])*(area*1000000))/1000000)* cof_aprov
                        else:
                            dict_altura['12']=((coeficiente*(max_fijo-datos_vista['12'])*(area*1000000))/1000000)* cof_aprov
                    dict_altura['anno'] = datos_vista['anno']
                    dict_altura['id'] = id
                    datos_generales.append(dict_altura)
                temp+=1
    ...
    def _valores_explotacion(self, id, anno=None):
        query = """SELECT
                          df_explotacion_cuenca_real.anno,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_enero, -999999.110)) as total_enero,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_febrero, -999999.110)) as total_febrero,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_marzo, -999999.110)) as total_marzo,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_abril, -999999.110)) as total_abril,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_mayo, -999999.110)) as total_mayo,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_junio, -999999.110)) as total_junio,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_julio, -999999.110)) as total_julio,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_agosto, -999999.110)) as total_agosto,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_septiembre, -999999.110)) as total_septiembre,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_octubre, -999999.110)) as total_octubre,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_noviembre, -999999.110)) as total_noviembre,
                          SUM(NULLIF(df_explotacion_cuenca_real.media_hiperanual_diciembre, -999999.110)) as total_diciembre
                    FROM
                          df_cuenca_subterranea,
                          df_explotacion_cuenca_real
                    WHERE
                          df_cuenca_subterranea.id = %s AND
                          df_explotacion_cuenca_real.cuenca_id = df_cuenca_subterranea.id"""
        if anno:
            query += """ AND df_explotacion_cuenca_real.anno = %s"""
        query += """ GROUP BY df_explotacion_cuenca_real.anno"""
                    ...
                    if datos[0]['anno'] == anno_inicio:
                        if mes_inicio < 2:
                            if anno_inicio == anno_fin and mes_fin < 1:
                                ...
                                arreglo_cuenca.append(dict_explotacion)
                                current_year += 1
                                continue
    ...
    def _valores_plan_explotacion(self, id, anno=None):
        query = """SELECT
                              df_explotacion_cuenca_plan.anno,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_enero, -999999.110)) as total_enero,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_febrero, -999999.110)) as total_febrero,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_marzo, -999999.110)) as total_marzo,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_abril, -999999.110)) as total_abril,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_mayo, -999999.110)) as total_mayo,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_junio, -999999.110)) as total_junio,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_julio, -999999.110)) as total_julio,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_agosto, -999999.110)) as total_agosto,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_septiembre, -999999.110)) as total_septiembre,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_octubre, -999999.110)) as total_octubre,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_noviembre, -999999.110)) as total_noviembre,
                              SUM(NULLIF(df_explotacion_cuenca_plan.media_hiperanual_diciembre, -999999.110)) as total_diciembre
                        FROM
                              df_cuenca_subterranea,
                              df_explotacion_cuenca_plan
    		            WHERE
                              df_cuenca_subterranea.id = %s AND
                              df_explotacion_cuenca_plan.cuenca_id = df_cuenca_subterranea.id"""
        if anno:
            query += """ AND df_explotacion_cuenca_plan.anno = %s"""
        query += """ GROUP BY df_explotacion_cuenca_plan.anno"""
    ...
    def calcular_delta_z1(self, id, vol_exp):
        """delta_z = vol_explotacion/coeficiente_almacenamiento*area"""
        val = None
        if vol_exp != None:
            obj_cuenca = self.env['df.cuenca.subterranea']
            #idd = obj_cuenca.search(cr,uid,[('id','=',[id])])
            #obj = obj_cuenca.browse(cr, uid, idd)[0]
            obj = obj_cuenca.search([('id','=',id)]) #cambio
            if obj.coeficiente_almacenamiento != 0 and obj.area != 0:
                val1 = (vol_exp*1000000)/(obj.coeficiente_almacenamiento * obj.area * 1000000)
                val = round(val1,2)
            else:
                 raise UserError(_('Debe entrar el área y el coeficiente de almacenamiento de la cuanca seleccionada.'))
    ...
    def calcular_nivel_lluvia_real(self,id,nreal,vol_exp_de_meses,anno,pozo_ids,metodo):
        ...
        if(obj.coeficiente_infiltracion!=0 and obj.area!=0 and obj.coeficiente_almacenamiento!=0):
            if (nreal.get('5') and nreal['5'] != None and nivel_real5!=None and vol_exp_de_meses['5']!=None):
                volumen5=((float(nreal['5'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
            ...
            if (nreal.get('6')and nreal['6'] != None and nivel_real6!=None and vol_exp_de_meses['6']!=None):
                volumen6=((float(nreal['6'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
            ...
            if (nreal.get('7')and nreal['7'] != None and nivel_real7!=None and vol_exp_de_meses['7']!=None):
                volumen7=((float(nreal['7'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
            ...
            if (nreal.get('8')and nreal['8'] != None and nivel_real8!=None and vol_exp_de_meses['8']!=None):
                volumen8=((float(nreal['8'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
            ...
            if (nreal.get('9')and nreal['9'] != None and nivel_real9!=None and vol_exp_de_meses['9']!=None):
                volumen9=((float(nreal['9'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
            ...
            if (nreal.get('10')and nreal['10'] != None and nivel_real10!=None and vol_exp_de_meses['10']!=None ):
                volumen10=((float(nreal['10'])/1000)*obj.coeficiente_infiltracion*obj.area*1000000) / (obj.coeficiente_almacenamiento*obj.area*1000000)
    ...
    def calcular_explotacion_plan(self, id,anno,tipo):
         ...
         if tipo == 'cuenca':
            ...
            obj_plan_explotacion = self.env['df.explotacion.cuenca.plan']
         res =  {'5': None,'6': None,'7': None,'8': None,'9':None,'10':None}
    ...
    def calcular_probabilidad(self,id,tipo):
        if tipo == 'cuenca':
            string_objeto_id = 'cuenca_id'
            obj_plan_explotacion = self.env['df.probabilidad.cuenca']
        probabilidad_ids_50=[]
        probabilidad_ids_75=[]
        probabilidad_ids_95=[]
        res =  {'5': None,'6': None,'7': None,'8': None,'9':None,'10':None}
        ...
        sql = """ select anno
                  ...
                  where tabla_objeto.cuenca_id = '""" + str(id) + """'
                  ORDER BY anno DESC;"""
        self.env.cr.execute(sql)
        datos_vistas = self.env.cr.dictfetchall()
        if datos_vistas:
            anno=datos_vistas[0]['anno']
            probabilidad_ids_50 = obj_plan_explotacion.search([('probabilidad','=', '50%'), (string_objeto_id, '=', id),('anno', '=', anno)]).ids
            probabilidad_ids_75 = obj_plan_explotacion.search([('probabilidad','=', '75%'), (string_objeto_id, '=', id),('anno', '=', anno)]).ids
            probabilidad_ids_95 = obj_plan_explotacion.search([('probabilidad','=', '95%'), (string_objeto_id, '=', id),('anno', '=', anno)]).ids
    ...
    def formar_diccionario(self,id,anno,tipo):
        if tipo == 'cuenca':
            string_objeto_id = 'cuenca_id'
            obj_lluvia_real = self.env['df.lluvia.real.cuenca']
    ...
    def calcular_niveles_pronosticos_reales(self, id, anno1,nreal,pronostico,tipo,metodo,pozo_ids=None):
        ...
        nivel_lluvia_real=self.calcular_nivel_lluvia_real(id,nreal,vol_exp_de_meses,anno1,pozo_ids,metodo)
        ...
        if nivel_abril:
            ...
            if (nivel_mayo or nivel_abril!=None)and pronostico!='puro':
                if fecha_mes5 > fecha_actual:
                    nivel_pron_mayo = self.calcular_niveles_pronosticos_mes(id, nivel_inicial_porcientos, vol_exp_de_meses['5'], 5, anno, lluvias_de_meses['5'])
                else:
            ...
            if (nivel_junio or nivel_mayo!=None) and pronostico!='puro':
                if fecha_mes6 > fecha_actual:
                    ...
                    nivel_pron_junio = self.calcular_niveles_pronosticos_mes(id, nivel_pron_mayo, vol_exp_de_meses['6'], 6, anno, lluvias_de_meses['6'])
                else:
            ...
            if (nivel_julio or nivel_junio!=None)and pronostico!='puro':
                if fecha_mes7 > fecha_actual:
                    ...
                    nivel_pron_julio = self.calcular_niveles_pronosticos_mes(id, nivel_pron_junio, vol_exp_de_meses['7'], 7, anno, lluvias_de_meses['7'])
            ...
            if (nivel_agosto or nivel_julio!=None)and pronostico!='puro':
                if fecha_mes8 > fecha_actual:
                    ...
                    nivel_pron_agosto = self.calcular_niveles_pronosticos_mes(id, nivel_pron_julio, vol_exp_de_meses['8'], 8, anno, lluvias_de_meses['8'])
                else:
            ...
            if (nivel_septiembre or nivel_agosto!=None)and pronostico!='puro':
                if fecha_mes9 > fecha_actual:
                    ...
                    nivel_pron_septiembre = self.calcular_niveles_pronosticos_mes(id, nivel_pron_agosto, vol_exp_de_meses['9'], 9, anno, lluvias_de_meses['9'])
                else:
            ...
            if (nivel_octubre or nivel_septiembre!=None)and pronostico!='puro':
                if fecha_mes10 > fecha_actual:
                    ...
                    nivel_pron_octubre = self.calcular_niveles_pronosticos_mes(id, nivel_pron_septiembre, vol_exp_de_meses['10'], 10, anno, lluvias_de_meses['10'])
                else:
        ...
        result = {
            ...
            'sigla': str(obj.codigo),
            'object':'',
            'categoryAxis': [],
            'yAxis': [{'minimo':0,'maximo':1000,'promedio_h_historico':0}],
            'valueAxis': [{'title': 'hm³', 'min': 99999999, 'max': 1000}],
            'series': [
                {'name': 'Nivel medido', 'data': [('May',self.calcular_nivel_real(id, 5, anno, pozo_ids,metodo)),('Jun',self.calcular_nivel_real(id, 6, anno, pozo_ids,metodo)),('Jul',self.calcular_nivel_real(id, 7, anno, pozo_ids,metodo)),('Agto',self.calcular_nivel_real(id, 8, anno, pozo_ids,metodo)),('Sep',self.calcular_nivel_real(id, 9, anno, pozo_ids,metodo)),('Oct',self.calcular_nivel_real(id, 10, anno, pozo_ids,metodo))]},
    ...
    def calcular_niveles_pronosticos_realesUnion(self, idd, anno1, vol_exp_de_meses,nreal,pronostico,tipo,lluvias_de_meses, nivel_abril, metodo,cont,meses1,pozo_ids=None):
        ...
        nivel_lluvia_real=self.calcular_nivel_lluvia_real(idd,nreal,vol_exp_de_meses,anno1,pozo_ids,metodo)
        nivel_inicial_porcientos = {'50': nivel_abril, '75': nivel_abril, '95': nivel_abril}
        if nivel_abril:
            ...
            if (nivel_mayo or nivel_abril!=None)and pronostico!='puro':
                # if fecha_mes5 > fecha_actual:
                    nivel_pron_mayo = self.calcular_niveles_pronosticos_mes(idd, nivel_inicial_porcientos, vol_exp_de_meses['5'], 5, anno, lluvias_de_meses['5'])
                # else:
                #     nivel_pron_mayo = {'50': None, '75': None, '95': None}
            else:
                nivel_pron_mayo = self.calcular_niveles_pronosticos_mes(idd, nivel_inicial_porcientos, vol_exp_de_meses['5'], 5, anno, lluvias_de_meses['5'])
            ...
            if (nivel_junio or nivel_mayo!=None)and pronostico!='puro':
                ...
                if cont!=1 and valor==5:
                    nivel_pron_mayo = {'50': nivel_mayo, '75': nivel_mayo, '95': nivel_mayo}
                nivel_pron_junio = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_mayo, vol_exp_de_meses['6'], 6, anno, lluvias_de_meses['6'])
                # else:
                #     nivel_pron_junio = {'50': None, '75': None, '95': None}
            elif nivel_pron_mayo:
                nivel_pron_junio = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_mayo, vol_exp_de_meses['6'], 6, anno, lluvias_de_meses['6'])
            ...
            if (nivel_julio or nivel_junio!=None)and pronostico!='puro':
                ...
                if cont!=1 and valor==6:
                    nivel_pron_junio = {'50': nivel_junio, '75': nivel_junio, '95': nivel_junio}
                nivel_pron_julio = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_junio, vol_exp_de_meses['7'], 7, anno, lluvias_de_meses['7'])
                # else:
                #     nivel_pron_julio = {'50': None, '75': None, '95': None}
            elif nivel_pron_junio:
                nivel_pron_julio = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_junio, vol_exp_de_meses['7'], 7, anno, lluvias_de_meses['7'])
            ...
            if (nivel_agosto or nivel_julio!=None)and pronostico!='puro':
                ...
                if cont!=1 and valor==7:
                    nivel_pron_julio = {'50': nivel_julio, '75': nivel_julio, '95': nivel_julio}
                nivel_pron_agosto = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_julio, vol_exp_de_meses['8'], 8, anno, lluvias_de_meses['8'])
                # else:
                #     nivel_pron_agosto = {'50': None, '75': None, '95': None}
            elif nivel_pron_julio:
                nivel_pron_agosto = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_julio, vol_exp_de_meses['8'], 8, anno, lluvias_de_meses['8'])
            ...
            if (nivel_septiembre or nivel_agosto!=None)and pronostico!='puro':
                ...
                if cont!=1 and valor==8:
                    nivel_pron_agosto = {'50': nivel_agosto, '75': nivel_agosto, '95': nivel_agosto}
                nivel_pron_septiembre = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_agosto, vol_exp_de_meses['9'], 9, anno, lluvias_de_meses['9'])
                # else:
                #     nivel_pron_septiembre = {'50': None, '75': None, '95': None}
            elif nivel_pron_agosto:
                nivel_pron_septiembre = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_agosto, vol_exp_de_meses['9'], 9, anno, lluvias_de_meses['9'])
            ...
            if (nivel_octubre or nivel_septiembre!=None)and pronostico!='puro':
                ...
                if cont!=1 and valor==9:
                    nivel_pron_septiembre = {'50': nivel_septiembre, '75': nivel_septiembre, '95': nivel_septiembre}
                nivel_pron_octubre = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_septiembre, vol_exp_de_meses['10'], 10, anno, lluvias_de_meses['10'])
                # else:
                #     nivel_pron_octubre = {'50': None, '75': None, '95': None}
            elif nivel_pron_septiembre:
                nivel_pron_octubre = self.calcular_niveles_pronosticos_mes(idd, nivel_pron_septiembre, vol_exp_de_meses['10'], 10, anno, lluvias_de_meses['10'])
        ...
        result = {
            ...
            'valueAxis': [{'title': 'hm³', 'min': 99999999, 'max': 1000}],
            'series': [
                {'name': 'Nivel medido', 'data': [('May',self.calcular_nivel_real(idd, 5, anno, pozo_ids,metodo)),('Jun',self.calcular_nivel_real(idd, 6, anno, pozo_ids,metodo)),('Jul',self.calcular_nivel_real(idd, 7, anno, pozo_ids,metodo)),('Agto',self.calcular_nivel_real(idd, 8, anno, pozo_ids,metodo)),('Sep',self.calcular_nivel_real(idd, 9, anno, pozo_ids,metodo)),('Oct',self.calcular_nivel_real(idd, 10, anno, pozo_ids,metodo))]},
                {'name': 'Nivel lluvia real', 'data': [('May',nivel_lluvia_real['5']),('Jun',nivel_lluvia_real['6']),('Jul',nivel_lluvia_real['7']),('Ago',nivel_lluvia_real['8']),('Sep',nivel_lluvia_real['9']),('Oct',nivel_lluvia_real['10'])]},
                # {'name': 'Nivel lluvia real', 'data': [('May',nreal['5']),('Jun',nreal['6']),('Jul',nreal['7']),('Ago',nreal['8']),('Sep',nreal['9']),('Oct',nreal['10'])]},
                {'name': 'Nivel lluvia 50%', 'data': [('May',res[0]['50']),('Jun',res[1]['50']),('Jul',res[2]['50']),('Ago',res[3]['50']),('Sep',res[4]['50']),('Oct',res[5]['50'])]},
                {'name': 'Nivel lluvia 75%', 'data': [('May',res[0]['75']),('Jun',res[1]['75']),('Jul',res[2]['75']),('Ago',res[3]['75']),('Sep',res[4]['75']),('Oct',res[5]['75'])]},
    ...
    def calcular_niveles_pronosticos_reales1(self, id, anno1,nreal,pronostico,abril,tipo,metodo,duracion,pozo_ids=None):
        """ SEGUN SE LLENAN LOS REALES SE UTILIZA ESE VALOR EN EL MES CORRESPONDIENTE
        ...
        while cont < 2:
            ...
            nivel_lluvia_real=self.calcular_nivel_lluvia_real(id,nreal,vol_exp_de_meses,anno,pozo_ids,metodo)
            nivel_inicial_porcientos = {'50': nivel_abril, '75': nivel_abril, '95': nivel_abril}
            if nivel_abril:
            ...
            #cambio sin realizar
            result['series'][0]['data'].append(['May.'+str(anno),self.calcular_nivel_real(id, 5, anno, pozo_ids,metodo)])
            result['series'][0]['data'].append(['Jun.'+str(anno),self.calcular_nivel_real(id, 6, anno, pozo_ids,metodo)])
            result['series'][0]['data'].append(['Jul.'+str(anno),self.calcular_nivel_real(id, 7, anno, pozo_ids,metodo)])
            result['series'][0]['data'].append(['Agto.'+str(anno),self.calcular_nivel_real(id, 8, anno, pozo_ids,metodo)])
            result['series'][0]['data'].append(['Sep.'+str(anno),self.calcular_nivel_real(id, 9, anno, pozo_ids,metodo)])
            result['series'][0]['data'].append(['Oct.'+str(anno),self.calcular_nivel_real(id, 10, anno, pozo_ids,metodo)])
        ...
        ## END METODOS PARA CALCULO DE NIVELES PRONOSTICOS
                        ...
                        if anno_inicio==recorrido.year:
                            if tipo == 'aritmetica':
                                if fecha_inicio == None and fecha_fin == None:
                                    ...
                                    datos = self.calcular_media_aritmetica(ids, pozo_ids, fecha_inicio[0], fecha_fin)
                                    niveles_ordenados = sorted(datos[temp], key=lambda tup: tup['anno'])
                                    datos_vistas=self.ordenar_diccionario(niveles_ordenados)
                            if tipo == 'formula':
                                if fecha_inicio == None and fecha_fin == None:
                                    ...
                                    datos_vistas = self.calcular_media_por_formula(ids, pozo_ids, fecha_inicio[0], fecha_fin)
                                    niveles_ordenados = sorted(datos_vistas[temp], key=lambda tup: tup['anno'])
                                    datos_vistas=self.ordenar_diccionario(cr, uid,niveles_ordenados)
                            for datos_vista in datos_vistas:
                                ...
                                for valor in datos_vista:
                                    ...
                                    if len(lista_sobrecarga)==2:
                                        verdadero=self.menor_proximo_valor(cr,lista_sobrecarga)
                                        sobrecarga=self.buscar_sobrecarga(cr,lista_sobrecarga,diferencia_sobrecarga)
                                        if verdadero:
                                    ...
                                    if contador_meses==24:
                                        if len(lista)==4:
                                            ...
                                            existe = self.obtener(cr,valor_anterior,lista)
                                            dic_recorridos={}
                                            cont_recorrido+=1
                                            dic_recorridos['mes']=existe['mes']
                                            dic_recorridos['anno']=existe['anno']
                        ...
                        if anno_fin==recorrido.year:
                            if tipo == 'aritmetica':
                                if fecha_inicio == None and fecha_fin == None:
                                    ...
                                    datos = self.calcular_media_aritmetica(ids, pozo_ids, fecha_inicio, fecha_fin[0])
                                    niveles_ordenados = sorted(datos[temp], key=lambda tup: tup['anno'],reverse = True)
                                    datos_vistas=self.ordenar_diccionario(niveles_ordenados)
                            if tipo == 'formula':
                                if fecha_inicio == None and fecha_fin == None:
                                    datos_vistas = self.calcular_media_por_formula(ids, pozo_ids, None, None)
                                    niveles_ordenados = sorted(datos_vistas[temp], key=lambda tup: tup['anno'],reverse = True)
                                    datos_vistas=self.ordenar_diccionario(niveles_ordenados)
                                else:
                                    datos_vistas = self.calcular_media_por_formula(ids, pozo_ids, fecha_inicio, fecha_fin[0])
                                    niveles_ordenados = sorted(datos_vistas[temp], key=lambda tup: tup['anno'],reverse = True)
                                    datos_vistas=self.ordenar_diccionario(niveles_ordenados)
                            for datos_vista in datos_vistas:
                                if encontro==1:
                                ...
                                while posicion>=1:
                                    ...
                                    if len(lista_sobrecarga)==2:
                                        ...
                                        sobrecarga=self.buscar_sobrecarga_inicio(lista_sobrecarga,diferencia_sobrecarga)
                                        if verdadero:
                                            if sobrecarga['sobrecarga']==1:
                                                dic_recorridos={}
                                                dic_recorridos['mes']=sobrecarga['mes']
    ...
    def calcular_explotacion(self, ids):
        """
        este nuevo metodo hala los valores de los sectores
        """

        exp_real_cuenca_obj = self.env['df.explotacion.cuenca.real']
        for id in ids:
            query = """SELECT
                    ...
                    WHERE
                          df_cuenca_subterranea.id = df_sector_hidrologico.cuenca_subterranea_id AND
                          df_sector_hidrologico.id = df_explotacion_sector_real.sector_id AND
                          df_cuenca_subterranea.id = %s
                    GROUP BY df_explotacion_sector_real.anno"""
            self.env.cr.execute(query, (id,))
            datos = self.env.cr.dictfetchall()
            for d in datos:
                vals = {
                    'anno': d['anno'],
                    'media_hiperanual_enero_string': d['total_enero'],
                    'media_hiperanual_febrero_string': d['total_febrero'],
                    'media_hiperanual_marzo_string': d['total_marzo'],
                    'media_hiperanual_abril_string': d['total_abril'],
                    'media_hiperanual_mayo_string': d['total_mayo'],
                    'media_hiperanual_junio_string': d['total_junio'],
                    'media_hiperanual_julio_string': d['total_julio'],
                    'media_hiperanual_agosto_string': d['total_agosto'],
                    'media_hiperanual_septiembre_string': d['total_septiembre'],
                    'media_hiperanual_octubre_string': d['total_octubre'],
                    'media_hiperanual_noviembre_string': d['total_noviembre'],
                    'media_hiperanual_diciembre_string': d['total_diciembre'],
                }
                exp_real_cuenca_ids = exp_real_cuenca_obj.search([('cuenca_id', '=', id), ('anno', '=', d['anno'])]).ids
                if exp_real_cuenca_ids:
                    exp_real_cuenca_obj.write([exp_real_cuenca_ids[0]], vals)  #cambio
                else:
                    vals.update({'cuenca_id': id})
                    exp_real_cuenca_obj.create(vals) #cambio

        return True

    def calcular_plan_explotacion(self, ids):
        """
        este nuevo metodo hala los valores de los sectores
        """

        exp_plan_cuenca_obj = self.env['df.explotacion.cuenca.plan']
        for id in ids:
            query = """SELECT
    		            ...
    		            WHERE
                              df_cuenca_subterranea.id = df_sector_hidrologico.cuenca_subterranea_id AND
                              df_sector_hidrologico.id = df_explotacion_sector_plan.sector_id AND
                              df_cuenca_subterranea.id = %s
            ...
            for d in datos:
                ...
                exp_plan_cuenca_ids = exp_plan_cuenca_obj.search([('cuenca_id', '=', id), ('anno', '=', d['anno'])]).ids
                if exp_plan_cuenca_ids:
                    exp_plan_cuenca_obj.write([exp_plan_cuenca_ids[0]], vals) #cambio
                else:
                    vals.update({'cuenca_id': id})
                    exp_plan_cuenca_obj.create(vals) #cambio
    ...
    def buscar_sectores_directo(self,pozo_ids):
        if pozo_ids:
            sql = """ SELECT
                          df_cuenca_subterranea.id AS cuenca_ids
                        FROM
                          public.df_pozo,
                          public.df_cuenca_subterranea
                        WHERE
                          df_pozo.cuenca_subterranea_id = df_cuenca_subterranea.id
                            AND
            ...
            lista=[]
            for datos_vista in datos_vistas:
                lista.append(datos_vista['cuenca_ids'])
        ...
        if pozo_ids:
            sql = """ SELECT
                        ...
                        WHERE
                          ...
                          df_bloque.sector_id = df_sector_hidrologico.id AND
                          df_sector_hidrologico.cuenca_subterranea_id = df_cuenca_subterranea.id
                            AND
                          df_pozo.id in %s AND df_pozo.representativo=True"""
            self.env.cr.execute(sql,(tuple(pozo_ids),))
            datos_vistas = self.env.cr.dictfetchall()
    ...
    def obtener_promedio_alturas(self,pozo_ids_actualizar,ok):
        ...
        cuenca_obj = self.env['df.cuenca.subterranea']
        pozo_obj= self.env['df.pozo']
        tramo_obj= self.env['df.tramo']
        #for cuenca in self.browse(cr,uid,sectores_directo):   #asi estaba
        for cuenca in self.browse(sectores_directo):
            vals={}
            suma_h_periodo = 0
            cantidad_h_periodo = 0
            obj_bloque = self.env['df.bloque']
            bloque_ids = obj_bloque.search([('sector_id.cuenca_subterranea_id', 'in', [cuenca.id])]).ids
            obj_sector = self.env['df.sector.hidrologico']
            sector_ids = obj_sector.search([('cuenca_subterranea_id', 'in', [cuenca.id])]).ids
            pozo_ids = pozo_obj.search(['|', ('bloque_id', 'in', bloque_ids), ('sector_hidrologico_id', 'in', sector_ids),('representativo','=',True)]).ids
            pozo_ids1 = pozo_obj.search([('representativo','=',True),('cuenca_subterranea_id', 'in', [cuenca.id])]).ids
            pozo_ids.extend([element for element in pozo_ids1 if element not in pozo_ids])
            if pozo_ids:
                inicio = tramo_obj.formar_fecha_inicio(pozo_ids)
                fin = tramo_obj.formar_fecha_fin(pozo_ids)
                if inicio != False and fin != False :
                    elementos = self.calcular_media_aritmetica([cuenca.id],pozo_ids, inicio, fin)
                    if elementos:
                        vals = {}
                        ...
                        while inicio <= fin:
                            tiempo = time.strptime(str(inicio.year) + '-' + str(inicio.month) + '-' + str(1),"%Y-%m-%d")
                            ...
                            if len(valores_recarga) > 0:  #si existen valores en analisis
                                ...
                                if valor - valor_anterior <= valor_precision and valor - valor_anterior > 0:
                                    ...
                                    suma_descargas_secuencia = 0
                                if (valor_anterior > valor or (valor - valor_anterior <= valor_precision and valor - valor_anterior > 0)) and suma_descargas_secuencia < valor_precision:
                                    #lo apilo
                                    ...
                                    while indice_desapilo >= 0:
                                        if valores_recarga[indice_desapilo]['valor'] > valores_recarga[indice_desapilo-1]['valor'] or valores_recarga[indice_desapilo]['valor'] > valores_recarga_punto_mas_alto:
                                    ...
                                    if len(valores_recarga) > 1:
                                        ...
                                        if (abs(valores_recarga[len(valores_recarga)-1]['valor'] - valores_recarga[0]['valor']) > valor_precision):
                                            if not (abs(valores_recarga[0]['valor'] - valores_recarga[len(valores_recarga) - 1][
                                            'valor'])== 0):
                                                suma_h_periodo += abs(valores_recarga[0]['valor']-valores_recarga[len(valores_recarga)-1]['valor'])
                        ...
                        rec_explotable=self.browse(cuenca.id).recurso_explotacion
                        area=self.browse(cuenca.id).area
                        deltaH=2*promedio
                        ...
                        if cuenca.id:
                            cuenca.write(vals) #cambio
                        else:
                            cuenca_obj.create(vals)
        return True


    def obtener_promedio_alturas_formula(self,pozo_ids_actualizar,ok):
        if ok:
            # ids=cuenca_obj.search(cr, uid, [])
            sectores_directo=self.buscar_sectores_directo(pozo_ids_actualizar)
            sectores_indirecto=self.buscar_sectores_indirecto(pozo_ids_actualizar)
            sectores_directo.extend([element for element in sectores_indirecto if element not in sectores_directo])
        else:
        ...
        for cuenca in self.browse(sectores_directo):
            ...
            if pozo_ids:
                ...
                fin = tramo_obj.formar_fecha_fin(pozo_ids)
                if inicio != False and fin != False :
                    elementos = self.calcular_media_por_formula([cuenca.id],pozo_ids, inicio, fin)
                    if elementos:
                        vals = {}
                        min = list(elementos[0][0].values())[0]
                        max = 0.0
